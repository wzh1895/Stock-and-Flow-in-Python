...
def fileLoad():
    ...
    if filename != '':
        lb.config(text = "File selected: " + filename)
        exp = SFDCanvas(filename)
    else:
        lb.config(text = "No file is selected.")
...
lb = Label(root,text='Use the button to load and display a Stella SD Model')
lb.pack()

btn1 = Button(root, text="Select file...",command = fileLoad)
btn1.pack()
